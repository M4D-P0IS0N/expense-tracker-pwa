...
- Export JSON before restoring a backup. Import validates the whole file (maximum 25 MB), accepts only application data keys, and never replaces authentication/session keys. Legacy deletion queues are recognized but never replayed. Cloud writes remain sequential: a failure is reported explicitly and may leave earlier batches restored; retrying uses upserts.
